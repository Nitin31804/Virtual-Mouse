    ...
    def calculate_palm_center(self) -> tuple:
        ...
        if wrist:
            ...
            if valid_points:
                ...
                avg_x = (wrist.x + sum(p.x for p in valid_points)) / (len(valid_points) + 1)
                avg_y = (wrist.y + sum(p.y for p in valid_points)) / (len(valid_points) + 1)
                return (avg_x, avg_y)
...
class HandTracker:
    ...
    def _process_hand_landmarks(self, landmarks, handedness, width, height) -> Optional[Hand]:
