...
class OverlayDisplay:
    ...
    def draw_active_region(
        self, image: np.ndarray, margin_ratio: float = 0.15
    ) -> np.ndarray:
        """
    ...
    def draw_hand_box(self, image: np.ndarray, hand) -> np.ndarray:
        ...
        cv2.putText(
            image, label, (x1, y1 - 5), self.font, 0.5, self.colors.background, 1
        )

        return image

    def draw_gesture_info(
        self, image: np.ndarray, gesture_name: str, hand
    ) -> np.ndarray:
        ...
        overlay = image.copy()
        cv2.rectangle(
            overlay, (x, y), (x + panel_w, y + panel_h), self.colors.background, -1
        )
        cv2.addWeighted(overlay, self.opacity, image, 1 - self.opacity, 0, image)

        # Text
        display_name = (
            gesture_name.replace("_", " ").upper() if gesture_name else "NEUTRAL"
        )
        ...
        cv2.putText(image, display_name, (x + 10, y + 60), self.font, 0.8, color, 2)
    ...
    def draw_calibration_info(
        self, image: np.ndarray, step: int, total_steps: int, instruction: str
    ) -> np.ndarray:
        """
        ...
        cv2.putText(
            image, instruction, (text_x + 2, text_y + 2), self.font, 1.0, (0, 0, 0), 2
        )
        cv2.putText(
            image, instruction, (text_x, text_y), self.font, 1.0, self.colors.warning, 2
        )
