...
class CursorController:
    def __init__(self, config: dict):
        ...
        self.frame_margin = config.get(
            "frame_margin", 0.25
        )  # Increased from 0.15 to make it easier to reach screen edges
    ...
    def _convert_to_screen_coordinates(
        ...
        screen_x = np.interp(
            x_norm, [self.frame_margin, 1 - self.frame_margin], [0, self.screen_width]
        )
        screen_y = np.interp(
            y_norm, [self.frame_margin, 1 - self.frame_margin], [0, self.screen_height]
        )
