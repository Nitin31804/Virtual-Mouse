...
from src.hand_detection.hand_tracker import HandTracker, Hand, HandLandmark, Handedness
...
class TestHandTracker:
    ...
    def hand_tracker(self, mock_config):
        ...
        with patch("src.hand_detection.hand_tracker.mp.solutions", create=True):
    ...
    def test_get_hand_size(self, hand_tracker):
        """Test calculating hand size"""
        landmarks = [HandLandmark(x=i*0.05, y=(i%5)*0.05, z=0.0, visibility=1.0) for i in range(21)]
        sample_hand = Hand(
            landmarks=landmarks,
            handedness=Handedness.RIGHT,
            bounding_box=(0.0, 0.0, 1.0, 1.0),
