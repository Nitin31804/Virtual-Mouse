...
from src.gestures.gesture_recognizer import GestureRecognizer, Gesture, GestureHistory
...
class TestGestureHistory:
    ...
    def test_add_same_gesture_duration(self, gesture_history):
        ...
        gesture = Gesture(
            name="cursor_move", finger_state=[0, 1, 0, 0, 0], confidence=0.9
        )
        ...
        gesture2 = Gesture(
            name="cursor_move", finger_state=[0, 1, 0, 0, 0], confidence=0.95
        )
    ...
    def test_history_size_limit(self, gesture_history):
        ...
        for i in range(10):
            gesture = Gesture(
                name=f"gesture_{i}", finger_state=[0, 1, 0, 0, 0], confidence=0.9
            )
            gesture_history.add(gesture)
...
class TestGestureRecognizer:
    ...
    def mock_gestures_config(self):
        return {
            "gestures": {
                "cursor_move": {"finger_state": [0, 1, 0, 0, 0], "priority": 1},
                "left_click": {"finger_state": [0, 1, 1, 0, 0], "priority": 2},
            }
        }
